...
Дата: 2026-06-01  
Версия: `0.0.23` (текущая) → `0.1.0` (после рефакторинга)
...
**Файлы токена:**  
После рефакторинга создать `tests/test_base/.syncbase` с токеном `y0__xCPzsuMAhiMzDkgs6fAixTU6apwTiR-C-v-SMjl-OEEJZkDpw`.
...
- [ ] **Имя файла-ключа:** `.syncbase` — ок? (Альтернативы: `.syncbase.key`, `syncbase.env`)
- [ ] **Формат файла-ключа:** одна строка токена или ini-формат `KEY=VALUE`?
- [ ] **Команда `init`:** добавлять её сразу или оставить ручное создание файла?
- [ ] **Версия:** `0.0.23` → `0.1.0` (значительный рефакторинг) — согласовать.
