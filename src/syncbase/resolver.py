"""Поиск ключа вольта .syncbase вверх по дереву файловой системы."""

from pathlib import Path
from typing import Optional, Tuple

VAULT_KEY_FILE = ".syncbase"
_TOKEN_KEY = "YANDEX_DISK_TOKEN"


def find_vault(start: Optional[Path | str] = None) -> Optional[Tuple[Path, str]]:
    """
    Ищет файл `.syncbase` начиная с `start` (по умолчанию — cwd)
    и поднимается вверх по дереву папок до корня файловой системы.

    Папка, в которой найден `.syncbase`, становится корнем вольта.

    Returns:
        Кортеж (vault_path, token), если файл найден и содержит токен.
    ...
    while True:
        candidate = current / VAULT_KEY_FILE
